  ...
  return (
    <AdminLayout title="Reviews" subtitle="Monitor all customer reviews and feedback">
      ...
      <div className="flex gap-1 mb-4 border-b border-slate-200 overflow-x-auto scrollbar-hide">
        {tabs.map(tab => (
          <button key={tab} onClick={() => setStatusFilter(tab)}
            className={`px-3 sm:px-4 py-2 text-sm font-medium capitalize rounded-t-lg border-b-2 -mb-px transition-colors whitespace-nowrap ${
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
        {filtered.length === 0 && (
          <div className="col-span-full text-center py-16 text-slate-400 bg-white rounded-2xl border border-slate-200">No reviews found</div>
        )}
        {filtered.map(r => (
          <div key={r.id} className={`bg-white rounded-2xl border shadow-sm p-4 sm:p-5 hover:shadow-md transition-shadow ${
            ...
            {/* Review text */}
            <p className="text-sm text-slate-600 mb-3 leading-relaxed line-clamp-3">"{r.text}"</p>

            {/* Service/Provider */}
            <div className="text-xs text-slate-400 flex items-center gap-2 mb-4 flex-wrap">
