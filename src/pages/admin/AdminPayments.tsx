      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm">
        <div className="flex flex-wrap items-center gap-3 px-5 py-4 border-b border-slate-100">
          ...
          <div className="flex gap-2 items-center">
            <input type="date" className="border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-cyan-400" />
            <span className="text-slate-400 text-sm">→</span>
            <input type="date" className="border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-cyan-400" />
          </div>
          <button className="flex items-center gap-2 text-sm text-slate-600 border border-slate-200 hover:bg-slate-50 px-3 py-2 rounded-lg transition-colors">
            ...
            <tbody className="divide-y divide-slate-50">
              ...
              {filtered.map(p => (
                <tr key={p.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-3.5 text-slate-800 font-medium">{p.customer}</td>
                  <td className="px-4 py-3.5 text-slate-500">{p.provider}</td>
                  <td className="px-4 py-3.5">
                    <span className="font-bold text-slate-800">Rs {p.amount.toLocaleString()}</span>
                  </td>
                  <td className="px-4 py-3.5"><StatusBadge status={p.method} /></td>
